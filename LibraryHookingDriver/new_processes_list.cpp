...
void NewProcessesList::add_process(ULONG process_id)
{
...
void NewProcessesList::remove_process(ULONG process_id)
{
	AutoLock lock(list_.get_mutex());
...
bool NewProcessesList::is_newly_created(ULONG process_id)
{
	AutoLock lock(list_.get_mutex());
